(function() {
    ...
    function fetchStackingFilterMap(callback) {(function() {
    'use strict';
...
function matchesExactBufferNumber(location, filter) {
   ...
   function processAndDisplay(containers) {
    ...
    containers.forEach(container => {
        ...
        const lane = stackingToLaneMap[stackingFilter] || 'N/A';
        const cpt = container.cpt || null;
        const contentCount = container.contentCount || 0; // Estrarre il content count dall'API
        ...
        if (
            ...
            if (!filteredSummary[lane][location]) {
                filteredSummary[lane][location] = { count: 0, contentCountTotal: 0, cpt: cpt };
            }

            filteredSummary[lane][location].count++;
            filteredSummary[lane][location].contentCountTotal += contentCount; // Somma i content count
        }
    });

    console.log("Filtered Summary with Content Count:", filteredSummary);
   ...
   function displayTable(sortedSummary) {
    ...
    Object.entries(sortedSummary).forEach(([lane, laneSummary]) => {
        ...
        const laneRow = $(`<tr class="laneRow" style="cursor: pointer;">
            <td colspan="4" style="font-weight: bold; text-align: left;">Lane: ${lane} - Totale Containers: ${laneTotal}, Content Count: ${laneContentCount}</td>
        </tr>`);
        ...
        Object.entries(laneSummary).forEach(([location, data]) => {
            ...
            row.append(`<td>${data.contentCountTotal}</td>`); // Mostra il totale content count per buffer
    ...
    $('body').append(contentContainer);
        ...
        $('#bufferFilterInput').val(selectedBufferFilter).on('keydown', function(event) {
    if (event.key === "Enter") {
        const manualFilter = $(this).val();
        if (manualFilter === '') {
            selectedBufferFilter = ''; // Clear the invisible filter if manually cleared
        } else {
            selectedBufferFilter = manualFilter;
        }
        fetchBufferSummary();
    }
});
...
function generatePieChart(filteredSummary) {
    ...
    Object.entries(filteredSummary).forEach(([lane, laneSummary]) => {
        Object.entries(laneSummary).forEach(([location, data]) => {
            if (location.startsWith("BUFFER")) {
                bufferLocations[location] = (bufferLocations[location] || 0) + data.count;
            }
        });
    });
            ...
            options: {
                ...
                onClick: function(event, elements) {
                    if (elements.length > 0) {
                        ...
                        const selectedLabel = chartData.labels[clickedElementIndex];
                        applyInvisibleFilter(selectedLabel); // Call the filter function
                    }
                }
            }
        });
    ...
    function applyInvisibleFilter(bufferFilter) {
    selectedBufferFilter = bufferFilter; // Apply the filter from the chart
    fetchBufferSummary(); // Refresh the data based on the selected filter
}
