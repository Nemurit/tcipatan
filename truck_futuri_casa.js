(function () {
    ...
    function updateRowCount(count) {
        if (!rowCountDisplay) {
            ...
            rowCountDisplay.style.top = '50px';
            rowCountDisplay.style.left = '10px';
            rowCountDisplay.style.zIndex = '10001';
            rowCountDisplay.style.backgroundColor = 'rgba(255, 255, 255, 0.9)';
        ...
        rowCountDisplay.innerHTML = `${count} righe trovate`;
    }
    ...
    function showButtonsAndInputs() {
        ...
        if (!timeInputBox) {
            ...
            timeInputBox.style.marginBottom = '10px';
        }

        if (!vrIdInputBox) {
            vrIdInputBox = document.createElement('input');
            vrIdInputBox.type = 'text';
            vrIdInputBox.placeholder = 'Filtra per VR ID';
            vrIdInputBox.style.marginBottom = '10px';
        }

        containermain = document.createElement('div');
        containermain.style.position = 'fixed';
        containermain.style.top = '20px';
    ...
    // Funzione per monitorare modifiche nell'iframe
    function monitorIframeChanges() {
        ...
        const observer = new MutationObserver(() => {
            console.log('Tabella aggiornata nell\'iframe');
            extractDataFromIframe(iframe, INITIAL_HOURS); // Ricarica i dati
        });

        const targetNode = iframeDoc.querySelector('table#dashboard');
        if (targetNode) {
            observer.observe(targetNode, {
                childList: true, // Rileva modifiche nei nodi figli
                subtree: true     // Rileva modifiche anche nei discendenti
            });
        }
    }
    ...
    window.onload = function () {
        createButtonForPageLoadAndDataExtraction();
        autoRefresh();
    };
})();
