...
function displayTable(sortedSummary) {
    ...
    const contentContainer = $('<div id="contentContainer" style="position: fixed; top: 10px; right: 10px; height: 90vh; width: 500px; overflow-y: auto; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); background: white; padding: 10px; border: 1px solid #ddd;"></div>');
    ...
    Object.entries(sortedSummary).forEach(([lane, laneSummary]) => {
        ...
        Object.entries(laneSummary).forEach(([location, data]) => {
            ...
            row.append(`<td>${data.cpt}</td>`);  // Mostra il valore di CPT
            tbody.append(row);
        });
